...
from stfno.fourier_transform_2d_layer_jit_torchCompile import (
...
class FNO2d_global(nn.Module):
    def __init__(
        ...
        width,
        T_in,
        total_vector_a_elements_i,
        T,
        total_vector_u_elements_i,
        number_of_layers,
        ...
        self.p_linears = nn.ModuleList(
            [
                nn.Linear((self.T_in) + 2, self.width)
                for i in range(self.total_vector_a_elements_i)
            ]
        )
        ...
        for j in range(self.n_layers):
            self.norm_linears_1.append(
                nn.ModuleList(
                    [
                        nn.InstanceNorm2d(self.width * self.mWidth_input_parameters[i])
                        for i in range(len(self.mWidth_input_parameters))
                    ]
                )
            )
        ...
        for j in range(self.n_layers):
            self.norm_linears_2.append(
                nn.ModuleList(
                    [
                        nn.InstanceNorm2d(self.width * self.nWidth_output_parameters[i])
                        for i in range(len(self.mWidth_input_parameters))
                    ]
                )
            )
        self.q_linears = nn.ModuleList(
            [
                MLP(
                    self.width * self.nWidth_output_parameters[i],
                    self.T_out * self.nWidth_output_parameters[i],
                    self.width * self.nWidth_output_parameters[i] * 4,
                )
                for i in range(len(self.nWidth_output_parameters))
            ]
        )
    ...
    def forward(self, x):
        ...
        for i, l in enumerate(self.p_linears):
            if i == len(self.p_linears) - 1:
                x1 = torch.cat(
                    (x[:, :, :, ((i - len(self.p_linears)) * self.T_in) :], grid),
                    dim=-1,
                )
                x1 = self.p_linears[i](x1)
                x = torch.cat(
                    (x[..., : ((i - len(self.p_linears)) * self.T_in)], x1), dim=-1
                )
            else:
                x1 = torch.cat(
                    (
                        x[
                            ...
                            :,
                            :,
                            ((i - len(self.p_linears)) * self.T_in) : (
                                (i + 1 - len(self.p_linears)) * self.T_in
                            ),
                        ],
                        grid,
                    ),
                ...
                x = torch.cat(
                    (
                        x[..., : ((i - len(self.p_linears)) * self.T_in)],
                        x1,
                        x[..., ((i + 1 - len(self.p_linears)) * self.T_in) :],
                    ),
                    dim=-1,
                )
        x = x.permute(0, 3, 1, 2)
        for i_n_layers in range(self.n_layers):
            ...
            for i in range(0, len(self.mWidth_input_parameters)):
                x2_tmp = [
                    x[
                        :,
                        self.width * self.input_parameter_order[i][j] : self.width
                        * (self.input_parameter_order[i][j] + 1),
                        :,
                        :,
                    ]
                    for j in range(len(self.input_parameter_order[i]))
                ]
                ...
                x[
                    :,
                    self.width * (i) : self.width
                    * (i + self.nWidth_output_parameters[i]),
                    :,
                    :,
                ] = x1[i] + x2[i]
                ...
                if i == 0:
                    x1 = self.q_linears[i](
                        x[
                            :,
                            self.width
                            * self.nWidth_output_parameters[i]
                            * (i) : self.width
                            * self.nWidth_output_parameters[i]
                            * (i + 1),
                            :,
                ...
                if i == 0 and len(self.nWidth_output_parameters) != 1:
                    ...
                    x = torch.cat(
                        (
                            x[
                                :,
                                : self.T_out * self.nWidth_output_parameters[i] * (i),
                                :,
                                :,
                            ],
                            self.q_linears[i](
    ...
    def get_grid(self, shape: Tuple[int, int, int], device):
