...
def readfiledata_h5py_ifilenoinSelectData(
    ...
    try:
        sample_field_name = fieldlist_parm_lst[0]
        if sample_field_name == 'e': sample_field_name_read = 'te'
        elif sample_field_name == 'i': sample_field_name_read = 'ti'
        else: sample_field_name_read = sample_field_name

        sample_file_path = os.path.join(path_data_read, 'data', f'{sample_field_name}_eq0_vec0.hdf5')
        
        with h5py.File(sample_file_path, 'r') as h5file:
            num_total_points = h5file[sample_field_name_read].shape[0]
        
        print(f"Dynamically found {num_total_points} data points in source file.")
        
        # Create a new list of file numbers covering all available points
        i_file_no_in_SelectData = list(range(num_total_points))
        i_file_no_in = i_file_no_in_SelectData # Ensure the old list is overwritten
        
    except Exception as e:
        print(f"Could not dynamically determine data size, falling back to original hardcoded lists. Error: {e}")
        # If dynamic check fails, fall back to the original hardcoded lists
        if if_HyperDiffusivity_case:
            if if_2ndRunHyperDiffusivity_case:
                ...
                i_file_no_in_SelectData = [45156,45230,45303,45377,45453,45589,45665,45799,45883,45970,46060,46200,46297,46398,46495,46590,46675,46750,46811,46918,47045,47121,47156,47199,47339,47504,47611,47792,47896,48069,48231,48398,48516,48636,48749,48859,49031,49156,49192,49375,49494,49677,49779,49943,50129,50249,50359,50457,50548,50644,50744,50914,51085,51156,51173,51255,51410,51567,51660,51813,51961,52050,52129,52214,52300,52393,52480,52572,52661,52745,52823,52907,52998,53091,53156,53182,53272,53362,53507,53591,53681,53778,53868,53954,54041,54184,54334,54489,54640,54723,54807,54889,54980,55137,55156,55288,55428,55581,55742,55899,56050,56139,56291,56382,56473,56619,56764,56853,57006,57099,57156,57189,57343,57496,57577,57654,57747,57869,57957,58034,58116,58193,58284,58378,58532,58621,58707,58860,59010,59102,59156,59259,59408,59565,59724,59800,59897,59990,60084,60224,60378,60528,60688,60838,60928,61022,61114,61156,61205,61296,61384,61476,61565,61659,61753,61852,61950,62046,62143,62231,62383,62543,62707,62803,62890,62979,63071,63156,63163,63258,63358,63451,63543,63629,63712,63864,64016,64105,64263,64350,64435,64521,64615,64708,64803,64899,64986,65074,65156,65158,65247,65339,65435,5531,65695,65837,65990,66153,66326,66473,66552,66634,66720,66889,67058,67156,67208,67352,67506,67674,67767,67857,67948,68016,68106,68259,68363,68473,68581,68684,68793,68893,68985,69089,69156,69205,69326,69444,69550,69678,69793,69921,70057,70187,70311,70439,70568,70672,70800,70931,71063,71156,71185,71310,71446,71565,71696,71829,71960,72085,72212,72341,72464,72681,72807,72937,73059,73156,73190,73312,73437,73568,73688,73814]
        else:
            i_file_no_in_SelectData = [334,667,1000,1334,1667,2001,2334,2667,3001,3334,3667,4001,5291,7291,9291,11291,14239,15952,18197,20279,20341,20405,20466,20533,20596,20654,20711,20775,20833,20890,20935,20988]
    ...
    for i_fieldlist_parm_lst, fieldlist_parm_lst_i in enumerate(fieldlist_parm_lst):
            ...
            for i_fieldlist_parm_eq_selected, fieldlist_parm_eq_selected in enumerate(range(fieldlist_parm_eq_range)):
                for i_fieldlist_parm_vector_i, fieldlist_parm_vector_i in  enumerate(range(fieldlist_parm_vector_lst[i_fieldlist_parm_lst])):
                    ...
                    data_read = data_read[()]
                    h5file.close()
                    # The code now uses all available data, so we slice it to fit the generated list
                    data_read_global[:,i_fieldlist_parm_lst,i_fieldlist_parm_eq_selected,i_fieldlist_parm_vector_i,:,:] = torch.from_numpy(data_read[:len(i_file_no_in),...]).to(data_read_global)
