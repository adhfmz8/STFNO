...
def initializationTrainTestParametersFile(
    sub_fieldlist_parm_eq_vector_train_global_lst_i_ii,
    j_fieldlist_parm_eq_vector_train_global_lst_i,
    data_read_global,
    train_a_range,
    train_u_range,
    test_a_range,
    ...
    sum_vector_a_elements_i_iter = len(
        sub_fieldlist_parm_eq_vector_train_global_lst_i_ii[
            j_fieldlist_parm_eq_vector_train_global_lst_i
        ][0]
    )
    ...
    if if_IncludeSteadyState:
        train_a_global_steadystate = torch.zeros(
            ntrain, sum_vector_a_elements_i_iter, S, S, T_in_steadystate
        )
        test_a_global_steadystate = torch.zeros(
            ntest, sum_vector_a_elements_i_iter, S, S, T_in_steadystate
        )
    sum_vector_u_elements_i_iter = len(
        sub_fieldlist_parm_eq_vector_train_global_lst_i_ii[
            j_fieldlist_parm_eq_vector_train_global_lst_i
        ][1]
    )
    train_u_global = torch.zeros(
        ntrain, sum_vector_u_elements_i_iter, S_r, S_theta, T_out
    )
    test_u_global = torch.zeros(
        ntest, sum_vector_u_elements_i_iter, S_r, S_theta, T_out
    )
    ...
    for i_ntrain in range(0, ntrain):
        for intermediate_i_file_no in train_a_range:
            ...
            for (
                k_fieldlist_parm_eq_vector_train_global_lst_i_j,
                fieldlist_parm_eq_vector_train_global_lst_i_j_k,
            ) in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
                item_of_sum_vector_train_a_elements_i += 1
                ...
                train_a_global[
                    i_ntrain,
                    item_of_sum_vector_train_a_elements_i,
                    :,
                    :,
                    (intermediate_i_file_no - (n_beg)),
                ] = data_read_global[
                    startofpatternlist_i_file_no_in_SelectData[(i_ntrain)]
                    + intermediate_i_file_no,
                ...
                if if_IncludeSteadyState:
                    for T_in_steadystate_iteration in range(T_in_steadystate):
                        train_a_global_steadystate[
                            i_ntrain,
                            item_of_sum_vector_train_a_elements_i,
                            :,
                            :,
                            T_in_steadystate_iteration,
                        ] = data_read_global[
                            startofpatternlist_i_file_no_in_SelectData[(i_ntrain)]
                            + T_in_steadystate_iteration,
                            fieldlist_parm_eq_vector_train_global_lst_i_j_0,
    ...
    for i_ntrain in range(0, ntrain):
        for intermediate_i_file_no in train_u_range:
            ...
            for (
                k_fieldlist_parm_eq_vector_train_global_lst_i_j,
                fieldlist_parm_eq_vector_train_global_lst_i_j_k,
            ) in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[1]):
                item_of_sum_vector_train_u_elements_i += 1
                if fieldlist_parm_eq_vector_train_global_lst_i_j_k[0] == "n":
                    fieldlist_parm_eq_vector_train_global_lst_i_j_0 = 0
                elif fieldlist_parm_eq_vector_train_global_lst_i_j_k[0] == "e":
                ...
                train_u_global[
                    i_ntrain,
                    item_of_sum_vector_train_u_elements_i,
                    :,
                    :,
                    (intermediate_i_file_no - (n_beg + (T_in * 1))) // 1,
                ] = data_read_global[
                    startofpatternlist_i_file_no_in_SelectData[(i_ntrain)]
                    + intermediate_i_file_no,
                    fieldlist_parm_eq_vector_train_global_lst_i_j_0,
    ...
    for i_ntest in range(0, ntest):
        for intermediate_i_file_no in test_a_range:
            ...
            for (
                ...
                test_a_global[
                    i_ntest,
                    item_of_sum_vector_test_a_elements_i,
                    :,
                    :,
                    (intermediate_i_file_no) // 1,
                ] = data_read_global[
                    startofpatternlist_i_file_no_in_SelectData[i_ntest + ntrain]
                    + intermediate_i_file_no,
                    fieldlist_parm_eq_vector_train_global_lst_i_j_0,
                    fieldlist_parm_eq_vector_train_global_lst_i_j_k[1],
                    fieldlist_parm_eq_vector_train_global_lst_i_j_k[2],
    ...
    for i_ntest in range(0, (ntest)):
        for intermediate_i_file_no in test_u_range:
            ...
            for (
                ...
                test_u_global[
                    i_ntest,
                    item_of_sum_vector_test_u_elements_i,
                    :,
                    :,
                    (intermediate_i_file_no - (n_beg + (T_in * 1))) // 1,
                ] = data_read_global[
    ...
    if if_IncludeSteadyState:
        train_a = torch.zeros(
            ...
            S_r,
            S_theta,
            (T_in + T_in_steadystate) * (sum_vector_a_elements_i_iter),
        )
        train_u = torch.zeros(
            ntrain, S_r, S_theta, T_out * (sum_vector_u_elements_i_iter)
        )
        test_a = torch.zeros(
            ntest,
            S_r,
        ...
        test_u = torch.zeros(
            ntest, S_r, S_theta, T_out * (sum_vector_u_elements_i_iter)
        )
    else:
        train_a = torch.zeros(
            ntrain, S_r, S_theta, T_in * (sum_vector_a_elements_i_iter)
        )
        train_u = torch.zeros(
            ntrain, S_r, S_theta, T_out * (sum_vector_u_elements_i_iter)
        )
    ...
    for item_of_sum_vector_a_elements_i_iter in range(sum_vector_a_elements_i_iter):
        train_a[
            ...
            :,
            :,
            (T_in * item_of_sum_vector_a_elements_i_iter) : (
                T_in * (item_of_sum_vector_a_elements_i_iter + 1)
            ),
        ] = train_a_global[:, item_of_sum_vector_a_elements_i_iter, :, :, :]
        test_a[
            :,
            :,
            :,
        ...
        ] = test_a_global[:, item_of_sum_vector_a_elements_i_iter, :, :, :]
    if if_IncludeSteadyState:
        for item_of_sum_vector_a_elements_i_iter in range(sum_vector_a_elements_i_iter):
            train_a[
                :,
                :,
                :,
                ((T_in + T_in_steadystate) * item_of_sum_vector_a_elements_i_iter) : (
                    ((T_in + T_in_steadystate) * (item_of_sum_vector_a_elements_i_iter))
                    + T_in
                    - 0
                ),
            ...
            train_a[
                ...
                :, item_of_sum_vector_a_elements_i_iter, :, :, :
            ]
            test_a[
                :,
                :,
    ...
    for item_of_sum_vector_u_elements_i_iter in range(sum_vector_u_elements_i_iter):
        train_u[
            ...
            :,
            :,
            (T_out * item_of_sum_vector_u_elements_i_iter) : (
                T_out * (item_of_sum_vector_u_elements_i_iter + 1)
            ),
        ] = train_u_global[:, item_of_sum_vector_u_elements_i_iter, :, :, :]
        test_u[
            :,
            :,
            :,
        ...
        ] = test_u_global[:, item_of_sum_vector_u_elements_i_iter, :, :, :]
    assert S_r == train_u.shape[-3]
    assert S_theta == train_u.shape[-2]
    if if_IncludeSteadyState:
        assert (
            T_in + T_in_steadystate
        ) * sum_vector_a_elements_i_iter == train_a.shape[-1]
        assert (T_in + T_in_steadystate) * sum_vector_a_elements_i_iter == test_a.shape[
            -1
        ]
    else:
    ...
    if if_IncludeSteadyState:
        ...
        test_a = test_a.reshape(
            ntest, S_r, S_theta, T_in * sum_vector_a_elements_i_iter
        )
    ...
    model_save_path_filename = (
        "i_fieldlist_parm_eq_vector_train_global_lst"
        + str(i_fieldlist_parm_eq_vector_train_global_lst)
        + "__ii_sub_fieldlist_parm_eq_vector_train_global_lst_i"
        + str(ii_sub_fieldlist_parm_eq_vector_train_global_lst_i)
        + "__j_fieldlist_parm_eq_vector_train_global_lst_i"
        + str(j_fieldlist_parm_eq_vector_train_global_lst_i)
    ...
    for (
        ...
        strn_epochs_dump_path_file1 = (
            strn_epochs_dump_path_file1
            + fieldlist_parm_eq_vector_train_global_lst_i_j_k[0]
            + "_"
            + str(fieldlist_parm_eq_vector_train_global_lst_i_j_k[1])
            + "_"
            + str(fieldlist_parm_eq_vector_train_global_lst_i_j_k[2])
        )
    ...
    strn_epochs_dump_path_file5 = (
        "data/"
        + "epochs_diagsTime_ntestZerosSize1only_"
        + str(i_fieldlist_parm_eq_vector_train_global_lst)
        + "__"
    )
    for (
    ...
    strn_epochs_dump_path_file6 = (
        "data/"
        + "epochs_diagsTime_jit_torchcompile"
        + str(i_fieldlist_parm_eq_vector_train_global_lst)
        + "__"
    )
    for (
