...
from readfiledata_h5py_i_file_no_in_SelectData import (
...
r_begn_theta0 = 2.108
r_end_theta0 = 2.442
globl_r_begn_estmt, globl_r_end_estmt = globl_r_begn_r_end_estmt_theta(
    S, nx_theta, r_cntr, r_begn_theta0, r_end_theta0
)
n_beg = 0
r_theta_phi, i_file_no_in, path = i_file_no_in_original(
    nx_r,
    nx_theta,
    theta_begn,
    theta_end,
    ...
    globl_r_begn_estmt,
    phi,
    if_HyperDiffusivity_case,
    if_2ndRunHyperDiffusivity_case,
    n_beg,
)
(
    fieldlist_parm_lst,
...
) = NIMROD_pde_operator_parameters_defination(if_2ndRunHyperDiffusivity_case, S)
data_dump_tmp = torch.zeros(S, S)
if_dumpfiledata = False
if if_dumpfiledata:
    dumpfiledata_h5py_i_file_no(
        fieldlist_parm_lst,
        fieldlist_parm_eq_range,
        fieldlist_parm_vector_lst,
        S,
...
if if_HyperDiffusivity_case:
    if if_2ndRunHyperDiffusivity_case:
        if S == 64:
            ...
            path_data_read = (
                "../../nimrod_hdf510_ZheBaihypdifusv_dump_data/npvjb_S32_dump_data/"
            )
    else:
        if S == 64:
            path_data_read = (
                "../../nimrod_hdf507_hypdifusv_dump_data/npvbj_S64_dump_data/"
            )
        else:
...
if if_readdumpfiledata:
    (
        ...
        fieldlist_parm_vector_lst,
        path_data_read,
        Option_NormalizingTrainTestData,
        i_file_no_in,
        OneByPowerTransformationFactorOfData,
    )
total_number_of_set_ntrain_plus_ntest_possible_i_file_no_in_SelectData = (
    len(i_file_no_in_SelectData) - T_in - T_out
)
if total_number_of_set_ntrain_plus_ntest_possible_i_file_no_in_SelectData < 0:
    exit(1)
ntrain = int(
    total_number_of_set_ntrain_plus_ntest_possible_i_file_no_in_SelectData
    * factor_ntrain_by_ntrainPlusntest
)
...
if sequential_splitting_option:
    ...
    startofpatternlist_i_file_no_in_SelectData = list(
        range(total_number_of_set_ntrain_plus_ntest_possible_i_file_no_in_SelectData)
    )
random.seed(random_seed_i_file_no_in_SelectData)
random.shuffle(startofpatternlist_i_file_no_in_SelectData)
if sequential_splitting_option:
    startofpatternlist_i_file_no_in_SelectData.extend(
        range(
            ntrain,
            total_number_of_set_ntrain_plus_ntest_possible_i_file_no_in_SelectData,
        )
...
print(
    ...
    [
        i_file_no_in_SelectData[i]
        for i in startofpatternlist_i_file_no_in_SelectData[:ntrain]
    ],
)
print(
    ...
    [
        i_file_no_in_SelectData[i]
        for i in startofpatternlist_i_file_no_in_SelectData[-ntest:]
    ],
)
